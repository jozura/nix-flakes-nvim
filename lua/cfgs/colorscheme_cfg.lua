...
function M.setup()
    require("black-metal").setup({
        ...
        highlights = {
            ["@lsp.type.macro.clojure"]     = { fg = "$string", fmt = 'none' },
            ["@keyword.import.clojure"]     = { fg = "$string", fmt = 'none' },
            ["@type"]                       = { fg = "$string", fmt = 'none' },
            ["@punctuation.special"]        = { fg = "$diag_green", fmt = 'none' },
            ["@lsp.type.namespace.clojure"] = { fg = "$diag_green", fmt = 'none' },
            ["@lsp.type.keyword.clojure"]   = { fg = "$diag_green", fmt = 'none' },
            ["@string.special.symbol"]      = { fg = "$diag_green", fmt = 'none' },
            ["@string.regexp"]              = { fg = "$diag_green", fmt = 'none' },
            ["@constant.builtin"]           = { fg = "$diag_green", fmt = 'none' },
            ["@function"]                   = { fg = "$type", fmt = 'none' },
            ["@variable"]                   = { fg = "$fg", fmt = 'none' },
            ["@punctuation.bracket"]        = { fg = "$fg", fmt = 'none' },
            ["@string"]                     = { fg = "$func", fmt = 'none' },
            ["GitSignsAdd"]                 = { fg = "green", fmt = 'none' },
            ["GitSignsChange"]              = { fg = "#948933", fmt = 'none' },
            ["GitSignsDelete"]              = { fg = "red", fmt = 'none' },
        },
    })
